...
%%
%% utkast_02 -- revised 2026-03-29
%% Compiled with: pdflatex -> biber -> pdflatex x 2

\documentclass[10pt]{article}

%% -- fix-cm: enable arbitrary CM math sizes (must come first) --------
\usepackage{fix-cm}
...
%% -- Layout: 130 x 200 mm pocketbok ---------------------------------
%%    Identical to Tractatus Formae / Gyldendal Fakkel format
\usepackage[
...
\usepackage{setspace}
\setstretch{1.15}                  % Tighter than 1.5; matches pocket format
\usepackage{titlesec}
\usepackage{enumitem}
\setlist{nosep, leftmargin=1.5em}  % Compact lists for small page
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}                 % For p{} columns

%% -- Font sizes matching Tractatus -----------------------------------
...
%% -- Page numbers: centered bottom, 9pt (Fakkel style) ---------------
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
...
%% -- Links (no bibliography) -----------------------------------------
\usepackage[hidelinks]{hyperref}
\hypersetup{hypertexnames=false}   % Fix duplicate PDF anchors
\usepackage{cleveref}

%% -- Emergency stretch for narrow text block -------------------------
...
%% -- Section formatting (small caps, centered -- Fakkel style) -------
\titleformat{\section}
...
%% -- Half-title (Fakkel convention) -----------------------------------
\thispagestyle{empty}
\vspace*{5cm}
\begin{center}
{\fontsize{14pt}{17pt}\selectfont\scshape Forml\ae re: Mathematica}
\end{center}
\newpage
...
\vspace{0.6cm}
{\normalsize\itshape The Axiomatic--Deductive Companion to\\[3pt]
Tractatus Formae}
\end{center}
\vfill
\begin{center}
...
The reader is assumed to be familiar with real analysis,
basic topology (compactness, continuity), and the calculus of
variations.

\medskip\noindent\textbf{Logical architecture.}
The theory rests on seven \emph{postulates}, empirically falsifiable
assumptions about the world, from which all theorems follow deductively.
The postulates are:
\begin{enumerate}[nosep]
  \item \textbf{Proposition~1.4}\; (Explanation requirement)
  ...
  \item \textbf{Postulate~8.1}\; (Path-dependent fitness)
\end{enumerate}

\medskip\noindent\textbf{Numbering note.}
The \textit{Mathematica} numbering (section.item) and the
\textit{Tractatus} numbering (decimal propositions) follow
different architectures. The correspondence table in the appendix
maps every formal element between the two systems.
...
$\Nav$ for a navigator (Def.~5.1),
and $\Pareto$ for the Pareto front (Thm.~2.8).
...
\[
  \bm{x}(a) = \bigl(x_1(a),\; x_2(a),\; \ldots,\; x_n(a)\bigr)
  \in \Rn.
\]
The \emph{morphospace} of class $C$ is the set of all
attribute vectors satisfying the physical and functional
constraints of $C$, equipped with the Euclidean metric
$d(\bm{x}, \bm{y}) = \norm{\bm{x} - \bm{y}}_2$.

Formally, let $\{g_j\}_{j=1}^{m}$ be a finite set of
continuous constraint functions
$g_j : \Rn \to \R$ encoding physical laws,
material limits, and functional requirements. Then:
\begin{equation}\label{eq:morphospace}
  \MC \;=\; \bigl\{\, \bm{x} \in \Rn
    \;\big|\; g_j(\bm{x}) \leq 0
    \;\;\forall\, j = 1, \ldots, m
  \,\bigr\}.
\end{equation}
Since each $g_j$ is continuous, $\MC$ is a closed subset
of\/ $\Rn$. If additionally the property ranges are bounded
(which holds for all physical objects), then $\MC$ is
\emph{compact}. \TF{1.1, 1.11, 1.12}
\end{definition}
...
\begin{definition}[Region typology]\label{def:1.2}
The full ambient space $\Rn$ partitions into three regions relative to
the class $C$:
\begin{enumerate}[label=(\roman*),nosep]
  \item \textbf{Busett (occupied) region} $B$:
    the support of the empirical measure $\mu$ on $\MC$
    induced by the population of realized objects.
    Formally, $B = \supp(\mu)$, which is closed in $\MC$.
...
\end{enumerate}
Thus $\MC = B \cup O$ with $B \cap O = \varnothing$, and
$\Rn = \MC \cup F = B \cup O \cup F$.  \TF{1.13}
\end{definition}

\begin{proposition}[Explanation requirement]\label{prop:1.4}
If $B$ contains more than one distinct position,
then the fact that a particular object $a$ occupies position
$\bm{x}(a)$ and not some other feasible position $\bm{y} \in \MC$
requires explanation.

\begin{proof}
Since $\MC$ is a compact subset of $\Rn$ with $n \geq 1$ and $B$
contains at least two distinct points,
there exist at least two distinct feasible positions. By the principle of
sufficient reason: if there were no cause favouring $\bm{x}(a)$
...
steepest improvement of the $i$-th criterion. \TF{2.11, 2.12}
\end{definition}
...
\[
  ...
  \bm{s}(\bm{x}) = \bigl(s_1(\bm{x}),\; \ldots,\;
  s_k(\bm{x})\bigr).
\]
Each component $s_i$ represents an independent criterion. \TF{2.14}
\end{definition}

\begin{definition}[Functional delimitation]\label{def:2.3}
Let $F_C : \Rn \to \{0, 1\}$ be the indicator function of the
functional requirement:
\[
  F_C(\bm{x}) =
  \begin{cases}
    1 & \text{if } \bm{x} \text{ satisfies the functional}\\
      & \text{requirement of class } C, \\
    0 & \text{otherwise.}
  \end{cases}
\]
...
\begin{postulate}[Multiple independent pressures]\label{post:2.4}
...
\begin{enumerate}[label=(\alph*),nosep]
  ...
  \item There exist at least two pressures $s_i, s_j$ that are
    \emph{statistically independent} with respect to the
    uniform measure on $\MC$: knowing $s_i(\bm{x})$
...
\begin{theorem}[Variation under constant function]\label{thm:2.5}
...
purpose). By Definition~\ref{def:2.3}, all objects in $C$ share the
same function, so $s^f(\bm{x})$ is constant for all
$\bm{x} \in \MC$ that satisfy the functional requirement. Then:
\begin{enumerate}[label=(\roman*),nosep]
...
\begin{proof}
\textit{(i)}\; By Definition~\ref{def:2.3}, every $\bm{x} \in B$
...
\textit{(ii)}\; By Postulate~\ref{post:2.4}, $k \geq 2$ and at least one
pressure $s^j$ ($j \neq f$) is independent of $s^f$. Since $s^f$ is
constant, the observed variation in $B$ (which is non-trivial since
$B$ contains more than one point) must be explained by the gradients of
the remaining pressures $s^j$, $j \neq f$. \qedhere
\end{proof}
\end{theorem}
...
Under Postulates~\ref{post:2.4} and \ref{post:2.6}, there exists no
...
\begin{proof}
Suppose for contradiction that $\bm{x}^*$ simultaneously maximizes
all $s_i$. Then $\grad s_i(\bm{x}^*) = \bm{0}$ for all
$i = 1, \ldots, k$, so $\bm{x}^*$ is a critical point of each
$s_i$ individually. In particular, $\bm{x}^*$ is the global
maximum of $s_i$ on $\MC$ (since we assumed it maximizes all pressures)
and separately the global maximum of $s_j$.

Since $\MC$ is compact and each $s_i$ is continuous, global maxima
exist (Weierstrass). Let $\bm{x}_i^* = \argmax_{\bm{x} \in \MC}
s_i(\bm{x})$ and $\bm{x}_j^* = \argmax_{\bm{x} \in \MC}
s_j(\bm{x})$. Our assumption requires
$\bm{x}^* = \bm{x}_i^* = \bm{x}_j^*$.

We show these individual maxima must be distinct, contradicting the
assumption. By Postulate~\ref{post:2.6}, there exists a region
$R \subseteq \MC$ with positive measure where
$\inner{\grad s_i}{\grad s_j} < 0$.

If $\bm{x}_i^* = \bm{x}_j^*$, consider the behaviour of
$s_i$ and $s_j$ near this common maximum. Near a non-degenerate
maximum, the Hessian is negative definite, so the direction of
steepest ascent to the maximum is unique from each nearby point.
But in $R$, moving toward the maximum of $s_i$ (in the direction
$\grad s_i$) necessarily moves away from the maximum of $s_j$
(since $\inner{\grad s_i}{\grad s_j} < 0$). If $R$ has positive
measure, there is an open set of points from which $s_i$ and $s_j$
cannot be simultaneously increased. In particular, any path from
$R$ to the common point $\bm{x}^*$ cannot simultaneously ascend
both $s_i$ and $s_j$, which contradicts $\bm{x}^*$ being the
maximum of both.

Therefore $\bm{x}_i^* \neq \bm{x}_j^*$, and no single point
maximizes all pressures simultaneously. Every realized form
balances competing demands: it is a compromise. \qedhere
\end{proof}
\end{theorem}
...
$B$ contains more than one point (Proposition~\ref{prop:1.4}),
...
$\bm{w} = (w_1, \ldots, w_k) \in \Delta^{k-1}$
(the $(k{-}1)$-simplex).
...
\[
  ...
  f(\bm{x}) = \Phi\bigl(s_1(\bm{x}),\; \ldots,\;
  s_k(\bm{x})\bigr)
\]
...
\[
  ...
  \;\subseteq\; \R^{n+1}.
\]
Local maxima of $f$ are \emph{peaks}. Local minima are
\emph{valleys}. Saddle points are \emph{ridges} separating peaks. \TF{3.2}
\end{definition}

\begin{theorem}[Multiple peaks]\label{thm:3.3}
Under Postulates~\ref{post:2.4} and \ref{post:2.6}, the fitness function
$f$ generically has at least two local maxima on $\MC$.

\begin{proof}
Let $s_i, s_j$ have conflicting gradients in a region $R$ of
positive measure (Postulate~\ref{post:2.6}).
Consider the one-parameter family of fitness functions
\[
  f_t(\bm{x}) = t \, s_i(\bm{x}) + (1-t) \, s_j(\bm{x})
  \quad\text{for } t \in [0,1].
\]
At $t = 1$, $f_1 = s_i$ attains its maximum at
$\bm{x}_i^* = \argmax s_i$. At $t = 0$, $f_0 = s_j$ attains its
maximum at $\bm{x}_j^*$. By Theorem~\ref{thm:2.7},
$\bm{x}_i^* \neq \bm{x}_j^*$.

We verify the hypotheses of the mountain-pass lemma. The functional
$f_t$ is continuous on the compact set $\MC$, so it satisfies the
Palais--Smale condition (every sequence $\{x_k\}$ with
$f_t(x_k)$ bounded and $\grad f_t(x_k) \to 0$ has a convergent
subsequence; this is automatic on a compact domain). For the
mountain-pass geometry: let $c_0 = f_t(\bm{x}_i^*)$ and
$c_1 = f_t(\bm{x}_j^*)$ be the values at the two endpoints.
The gradient conflict in $R$ implies that $f_t$ is strictly
below $\max(c_0, c_1)$ on a neighbourhood separating the two maxima,
since moving from one peak toward the other requires simultaneously
descending in at least one of $s_i, s_j$.

Define $\Gamma = \{\gamma \in C([0,1], \MC) :
\gamma(0) = \bm{x}_i^*,\; \gamma(1) = \bm{x}_j^*\}$.
By the mountain-pass theorem, the minimax value
\[
  c = \inf_{\gamma \in \Gamma}
  \max_{s \in [0,1]} f_t(\gamma(s))
\]
is a critical value of $f_t$, realized at a saddle point
$\bm{x}_s$ with $f_t(\bm{x}_s) = c$. This saddle separates the
basins of $\bm{x}_i^*$ and $\bm{x}_j^*$.

For the aggregated fitness $f = \Phi(\bm{s})$ with $\Phi$
monotone and smooth, the same structure persists: the basins of
$\bm{x}_i^*$ and $\bm{x}_j^*$ in the individual pressures create
distinct basins in $f$. Each basin of $f$ is a compact connected
subset of $\MC$; since $f$ is continuous on each, it attains a
maximum there (Weierstrass). Hence $f$ has at least two local
maxima on $\MC$. \qedhere
\end{proof}
\end{theorem}
...
\begin{corollary}[Analyst-dependence of style decomposition]%
\label{cor:3.5}
...
(final form). Continuity guarantees local increments:
for every $\varepsilon > 0$ there exists $\delta > 0$ such that
$|t' - t| < \delta$ implies
$\norm{\gamma(t') - \gamma(t)} < \varepsilon$.
The path is \emph{history-dependent}: the accessible region at time $t$
depends on $\gamma\bigl([0, t]\bigr)$; this is formalized
variationally in Postulate~\ref{post:8.1}. \TF{3.6}
\end{definition}
...
long periods of gradual drift punctuated
...
\begin{definition}[Navigator]\label{def:5.1}
A \emph{navigator} is a triple
$\Nav = (G,\; \mu,\; \alpha)$
where:
\begin{enumerate}[label=(\alph*),nosep]
  \item $G = G\bigl(\gamma([0,t])\bigr) \subseteq \MC$ is a
    \emph{goal set} (target region): a non-empty, possibly
    trajectory-dependent subset of morphospace that the navigator
    steers toward. The dependence on the trajectory history
    $\gamma([0,t])$ allows for goal revision, goal construction,
    and goal stacking across scales.
  \item $\mu : \MC \to \R_{\geq 0}$ is a \emph{distance function}:
    $\mu(\bm{x}) = d(\bm{x},\, G)$ for a metric $d$ on $\MC$, with
    $\mu(\bm{x}) = 0 \iff \bm{x} \in G$.
  \item $\alpha : \MC \to \Rn$ is a \emph{steering field}: a
    vector field on $\MC$ (in the ambient $\Rn$ sense)
    satisfying the \emph{time-averaged descent condition}:
    for every $t$ with $\mu(\gamma(t)) > 0$ and every
    characteristic timescale $\Delta > 0$,
    \begin{equation}\label{eq:steering}
      \int_t^{t+\Delta}
        \inner{\alpha(\gamma(\tau))}{-\grad\mu(\gamma(\tau))}
      \, d\tau \;>\; 0.
    \end{equation}
    That is, $\alpha$ has a net component pointing toward
    decreasing distance to $G$, averaged over any interval,
    while permitting local exploratory deviations.
\end{enumerate}

\medskip\noindent
Three conditions are required: (a)~the system has a goal state,
(b)~it can register distance from the goal, and (c)~it can adjust
toward the goal on average. No consciousness, intention, or nervous
system is required. A thermostat, a gradient-descent algorithm, and a
chemical reaction driven toward equilibrium are all navigators. \TF{5.1, 5.11}
\end{definition}

\begin{definition}[Cognitive light cone]\label{def:5.2}
The \emph{cognitive light cone} (boundary) of a navigator
$\Nav$ at time $t$ is the maximal region of morphospace--time
in which $\Nav$ can represent states and steer:
\begin{multline}\label{eq:lightcone}
  \partial\Nav \;=\;
  \bigl\{\, (r,\, \tau) \in \MC \times \R
    \;\big|\; \\
    \Nav \text{ can represent and steer}
    \text{ toward states in } (r, \tau)
  \,\bigr\}.
\end{multline}
...
\begin{definition}[Intelligence]\label{def:5.3}
The \emph{intelligence} of a navigator $\Nav$ is its capacity to avoid
local optima in the fitness landscape:
\[
  I(\Nav) \;\propto\;
  \max\bigl\{\, d\bigl(\gamma_\Nav(t),\;
    \bm{x}_{\mathrm{local}}\bigr)
    \;\big|\;
    \gamma_\Nav \to \bm{x}_{\mathrm{global}}
  \,\bigr\}
\]
...
\newpage
%% =====================================================================
%% CHAPTER 6: EMPIRICAL -- MATERIAL SIGNATURES
%% =====================================================================
\section{Empirical: Material Signatures}\label{sec:empirical}

\begin{postulate}[Geometric material signature]\label{post:6.1}
...
\begin{theorem}[Information theorem]\label{thm:6.2}
...
By Postulate~\ref{post:6.1}, $P(\bm{x} \mid m) \neq P(\bm{x})$ for
...
\]
Since $G \not\perp\!\!\!\perp M$ (by Postulate~\ref{post:6.1}), we
have $\MI(G;\, M) > 0$.
...
\newpage
%% =====================================================================
%% CHAPTER 7: SUBSTRATE INDEPENDENCE AND EMERGENCE
%% =====================================================================
\section{Substrate Independence and Emergence}%
\label{sec:substrate}

\begin{postulate}[Substrate-independent navigation]\label{post:7.1}
The conditions (a), (b), (c) in Definition~\ref{def:5.1} can be
...
\begin{theorem}[Position invariance of morphospace]%
\label{thm:7.2}
...
\begin{remark}
Theorem~\ref{thm:7.2} establishes that the \emph{output space}
(morphospace) is substrate-invariant: position is determined by
measurable properties, not production history. It does not
establish that any specific navigational competency can be realized
in multiple substrates. The latter is the content of
Postulate~\ref{post:7.1}, which remains an empirically motivated
assumption. The framework provides substrate-neutral vocabulary;
substrate independence of capacity is postulated, not proved.
\end{remark}

\begin{conjecture}[Emergent navigation]\label{conj:7.3}
...
When sub-navigators interact through shared selection pressures,
...
$\dot{\bm{y}} = \Pi_* \dot{\bm{X}}$ satisfy Definition~\ref{def:5.1}
with $(G, \mu, \alpha)$ that cannot be decomposed as functions of any
single $(G_j, \mu_j, \alpha_j)$.

The conditions on $\phi_{jl}$ under which such a map $\Pi$ exists
(e.g., sufficient coupling strength, symmetry, or mean-field limits)
remain to be established.
\end{conjecture}

\begin{remark}[Empirical motivation]\label{rem:xenobots}
Synthetic organisms assembled from frog embryonic cells
...
dynamics. This demonstrates that collective triples $(G, \mu, \alpha)$
arise as real emergent properties, not reducible to individual cell
navigation. Similarly, the final form of a collaboratively designed
object is not specified by any single participant but emerges from
negotiation between them. These examples motivate the conjecture
but do not constitute a mathematical proof of it.
\TF{5.6, 5.61, 5.62}
\end{remark}
...
\begin{postulate}[Path-dependent fitness]\label{post:8.1}
...
\]
where $g : \MC \times \Rn \times \R \to \R$ is a Lagrangian that
...
\begin{theorem}[Stationary path]\label{thm:8.2}
...
\begin{align*}
  ...
  \right) \cdot \eta \; dt\\
  &\quad+ \left[\frac{\partial g}{\partial \dot{\gamma}}
  \cdot \eta \right]_0^T.
\end{align*}
...
\begin{theorem}[Path modifies landscape]\label{thm:8.3}
...
Since $\eta_i$ is continuous on the compact set $\MC \times \MC$,
it is integrable along any continuous path $\gamma$.
...
\begin{theorem}[Spreading relation]\label{thm:8.4}
...
inverse temperature). This distributional form is an additional
modeling assumption, not derived from the preceding postulates;
it is motivated by the analogy between fitness-driven selection
and statistical-mechanical equilibrium.

Near the maximum, expand $f$ to second order:
\[
...
\begin{theorem}[Provisional compromise]\label{thm:9.1}
...
\begin{theorem}[Durable traditions]\label{thm:9.2}
...
adaptation requires adjusting only
$\Delta k \leq k_\mathcal{T}$ mechanisms.
...
\begin{theorem}[Falsifiability of the framework]\label{thm:9.3}
...
within a functional class $C$, then Postulate~\ref{post:2.4} fails, and
with it Theorems~\ref{thm:2.5}, \ref{thm:2.7}, \ref{thm:2.8},
\ref{thm:3.3}, Corollary~\ref{cor:3.5}, and Theorem~\ref{thm:9.1}.

\begin{proof}
The deductive chain is:
\begin{enumerate}[nosep]
  \item Postulate~\ref{post:2.4} (multiple independent pressures)
    $\implies$ Theorem~\ref{thm:2.5} (variation not explained by
    function).
  \item Postulates~\ref{post:2.4} + \ref{post:2.6} (conflicting
  ...
  \item Theorem~\ref{thm:3.3} + Definition~\ref{def:3.4} $\implies$
    Corollary~\ref{cor:3.5} (analyst dependence).
  \item Postulate~\ref{post:4.1} (dynamic landscape) +
    Theorem~\ref{thm:2.7}
    $\implies$ Theorem~\ref{thm:9.1} (provisional compromise).
\end{enumerate}

If any of the seven postulates can be empirically disproven, the
...
``form follows function'' was unfalsifiable and therefore
not a scientific claim. This framework makes quantitative predictions
(Theorem~\ref{thm:8.4}, Postulate~\ref{post:6.1}) that can be tested
against data.
...
\medskip
{\small
\begin{longtable}{@{}p{50mm}lp{18mm}@{}}
\toprule
\textsc{Mathematica} & \textsc{Type} & \textsc{TF} \\
\midrule
\endfirsthead
\toprule
\textsc{Mathematica} & \textsc{Type} & \textsc{TF} \\
...
Def.~1.1 (Morphospace) & Def. & 1.1 \\
Def.~1.2 (Region typology) & Def. & 1.13 \\
Prop.~1.4 (Explanation) & Prop. & 1.14 \\
\addlinespace
Def.~2.1 (Selection pressure) & Def. & 2.1 \\
Def.~2.2 (Pressure vector) & Def. & 2.14 \\
Def.~2.3 (Functional delim.) & Def. & 2.4 \\
Post.~2.4 (Multiple pressures) & Post. & 2.2 \\
Thm.~2.5 (Variation under fn.) & Thm. & 2.21 \\
Post.~2.6 (Conflicting grad.) & Post. & 2.3 \\
Thm.~2.7 (Compromise) & Thm. & 2.31 \\
Thm.~2.8 (Multiple comp.) & Thm. & 2.32 \\
\addlinespace
Def.~3.1 (Fitness function) & Def. & 3.1 \\
Def.~3.2 (Fitness landscape) & Def. & 3.2 \\
Thm.~3.3 (Multiple peaks) & Thm. & 3.3 \\
Def.~3.4 (Style) & Def. & 3.4 \\
Cor.~3.5 (Analyst dependence) & Cor. & 3.41 \\
Def.~3.6 (Path / wandering) & Def. & 3.6 \\
\addlinespace
Post.~4.1 (Dynamic landscape) & Post. & 4.1 \\
Thm.~4.2 (Punctuated equil.) & Thm. & 4.11 \\
Def.~4.3 (Landscape memory) & Def. & 4.2 \\
\addlinespace
Def.~5.1 (Navigator) & Def. & 5.1 \\
Def.~5.2 (Cognitive light cone) & Def. & 5.2 \\
Def.~5.3 (Intelligence) & Def. & 5.3 \\
\addlinespace
Post.~6.1 (Geometric signature) & Post. & 8.1 \\
Thm.~6.2 (Information theorem) & Thm. & 8.11 \\
\addlinespace
Post.~7.1 (Substrate indep.) & Post. & 5.12 \\
Thm.~7.2 (Position invariance) & Thm. & 1.4 \\
Conj.~7.3 (Emergent navig.) & Conj. & 5.62 \\
\addlinespace
Post.~8.1 (Path-dep.\ fitness) & Post. & 7.1 \\
Thm.~8.2 (Stationary path) & Thm. & 7.11 \\
Thm.~8.3 (Path modifies land.) & Thm. & 7.12 \\
Thm.~8.4 (Spreading relation) & Thm. & 7.2 \\
\addlinespace
Thm.~9.1 (Provisional comp.) & Thm. & 7.4 \\
Thm.~9.2 (Durable traditions) & Thm. & 7.5 \\
Thm.~9.3 (Falsifiability) & Thm. & 7.7 \\
\end{longtable}
}
...
The deductive structure of the theory, showing which results
depend on which postulates and earlier theorems:
...
\begin{verbatim}
             ...
             |
POST 2.4 ---+-- Def 2.3 (Functional delim.)
(k >= 2)    |    +-- Thm 2.5 (Var /= function)
             |
             |         +-- Thm 2.7 (Compromise)
POST 2.6 ---+---------+
(conflict)  |         +-- Thm 2.8 (Mult. comp.)
             |              |
             |        Thm 3.3 (Multiple peaks)
             |         +-- Def 3.4 (Style)
             |         +-- Cor 3.5 (Analyst dep.)
             |
POST 4.1 ---+-- Thm 4.2 (Punct. equil.)
(dynamic)   |   +-- Def 4.3 (Memory)
             |
             +-- Thm 9.1 (Provis. comp.)
             |       [from 2.7 + 4.1]
             +-- Thm 9.2 (Durable trad.)
                     [from 9.1 + 5.1]

POST 6.1 ---- Thm 6.2 (Information thm)
(material)

POST 7.1 ---- Thm 7.2 (Pos. invariance)
(substr.)      +-- Conj 7.3 (Emergence)

POST 8.1 ---+-- Thm 8.2 (Stat. path / E-L)
(path-dep.)  +-- Thm 8.3 (Path mod. land.)
             +-- Thm 8.4 (Spreading rel.)

Thm 9.3 (Falsifiability)
  [meta: Post 2.4 fails ->
    2.5, 2.7, 2.8, 3.3, 9.1 fall]
\end{verbatim}
\end{scriptsize}
...
\begin{itemize}[nosep]
  \item 12 definitions (Def.~1.1--5.3)
  \item 7 postulates (Post.~2.4, 2.6, 4.1, 6.1, 7.1, 8.1
    + the explanation principle Prop.~1.4)
  \item 11 theorems (Thm.~2.5--9.3)
  \item 1 corollary (Cor.~3.5)
  \item 1 conjecture (Conj.~7.3)
\end{itemize}
