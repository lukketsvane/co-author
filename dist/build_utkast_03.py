...
OUTPUT_DOCX = os.path.join(BASE_DIR, "utkast_03.docx")
OUTPUT_PDF = os.path.join(BASE_DIR, "utkast_03.pdf")


def get_prop_indent(num_str):
    """Get left indent in cm based on proposition number depth.
    Tractatus style: deeper propositions get more indentation.
    Main props (1, 2...) = flush left. 1.1 = 0.6cm. 1.11 = 1.2cm. etc.
    """
    if '.' not in num_str:
        return 0.0
    decimal = num_str.split('.')[1]
    return len(decimal) * 0.6
    ...
    for btype, bdata in blocks:
        if btype == 'heading':
            ...
            if prop_match:
                ...
                add_formatted_text(p, rest, base_font='Palatino Linotype')
            else:
                add_formatted_text(p, bdata, base_font='Palatino Linotype')
...
def build_pdf(md_text):
    ...
    # Post-process HTML: style proposition numbers with Tractatus-style indentation
    def prop_replacement(m):
        num = m.group(1)
        tag = m.group(2)
        if '.' not in num:
            depth = 0
            ...
            decimal = num.split('.')[1]
            depth = min(len(decimal), 3)
        return f'</p><p class="prop prop-depth-{depth}"><span class="prop-num">{num}</span> <span class="prop-tag">[{tag}]</span>'
...
body {{
    font-family: 'Palatino Linotype', Palatino, 'EB Garamond', 'DejaVu Serif', Georgia, serif;
    font-size: 11pt;
    line-height: 1.5;
    color: #111;
    hyphens: auto;
...
/* Tractatus-style indentation by proposition depth */
.prop {{
    margin-top: 0.5em;
    margin-bottom: 0.2em;
    text-align: justify;
}}
.prop-depth-0 {{ margin-left: 0; }}
.prop-depth-1 {{ margin-left: 0.6cm; }}
.prop-depth-2 {{ margin-left: 1.2cm; }}
.prop-depth-3 {{ margin-left: 1.8cm; }}
.prop-num {{
    font-weight: bold;
    font-size: 11pt;
}}
